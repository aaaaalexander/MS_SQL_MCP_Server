...
try:
    ...
    logger.info("Successfully imported FastMCP for analyze tools.")
except ImportError as e:
    logger.error(f"Failed to import FastMCP: {e}")

# Get reference to the mcp instance from sql_mcp_server.py
import sys
import importlib.util
server_spec = importlib.util.find_spec("sql_mcp_server")
if server_spec:
    server_module = importlib.util.module_from_spec(server_spec)
    server_spec.loader.exec_module(server_module)
    mcp = getattr(server_module, "mcp", None)
    _get_db_connection_blocking = getattr(server_module, "_get_db_connection_blocking", None)
    _execute_query_blocking = getattr(server_module, "_execute_query_blocking", None)
    if not mcp:
        logger.error("Could not find 'mcp' in sql_mcp_server.py")
    if not _get_db_connection_blocking or not _execute_query_blocking:
        logger.error("Could not find required database functions in sql_mcp_server.py")
else:
    logger.error("Could not find sql_mcp_server.py module")
    mcp = None
